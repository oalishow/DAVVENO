...
export default function MemberList() {
  ...
  const loadMembers = async () => {
    ...
    try {
      ...
      setMembers(loaded.filter(m => !m.deletedAt));
    } catch (e) {
      console.error(e);
    } finally {
      setLoading(false);
  ...
  if (members.length === 0) {
    return <p className="text-slate-500 italic p-6 text-center text-sm">Nenhum registo na base de dados.</p>;
  }

  return (
    <>
      <div className="space-y-2 max-h-[300px] sm:max-h-[400px] overflow-y-auto custom-scrollbar pr-2 mt-4">
        <div className="text-xs sm:text-sm font-medium text-sky-700 dark:text-sky-300 bg-sky-100 dark:bg-sky-500/10 px-3 py-1 mb-2 rounded-full border border-sky-200 dark:border-sky-500/20 inline-block">
