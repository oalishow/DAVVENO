...
export default function App() {
  ...
  return (
    <div className="min-h-screen relative flex items-center justify-center p-4">
      <button 
        onClick={toggleTheme}
        className="absolute top-4 right-4 p-2.5 rounded-full bg-white/80 dark:bg-slate-800/80 text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 shadow-sm backdrop-blur-sm z-[100] transition-colors"
      >
        {theme === 'dark' ? <Sun className="w-5 h-5" /> : <Moon className="w-5 h-5" />}
      </button>

      <div className="w-full max-w-3xl glass-panel rounded-2xl sm:rounded-3xl p-5 sm:p-8 md:p-10 animated-fade-in relative overflow-hidden">
        ...
        <div className="relative z-10 space-y-6 sm:space-y-8">
          ...
          {/* Abas de Navegação */}
          <div className="flex bg-slate-100/80 dark:bg-slate-900/60 rounded-xl p-1.5 shadow-inner border border-slate-200 dark:border-slate-700/50">
            <button 
